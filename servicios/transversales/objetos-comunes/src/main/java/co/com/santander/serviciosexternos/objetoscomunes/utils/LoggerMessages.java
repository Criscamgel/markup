...
public class LoggerMessages {
	 ...
	 * @param endpoint
	 */
	public static void generateInitialLog(ClientesServiciosEnum client, RequestHeaderDTO request, String jsonBody, String endpoint) {
		// Log of request header
		log.info(client.toString().toLowerCase() + getCallingInformationObject(request, endpoint, true));
		// Log of request body
		log.info(client.toString().toLowerCase() + getCallingInformationObject(jsonBody == null ? String.valueOf("") : jsonBody, endpoint, false));
	}
	 ...
	 * @param fromCalling
	 */
	public static void generateInitialLogSimple(ClientesServiciosEnum client, String fromCalling) {
		log.info(client.toString().toLowerCase().concat(CALLING_CLIENT_FROM.concat(fromCalling.concat("."))));
	}
	 ...
	 * @param request
	 */
	public static void generateFinalLog(ClientesServiciosEnum client, boolean isError, String message, RequestHeaderDTO request) {
		if(!isError)
			log.info(client.toString().toLowerCase() + matchRequestWithResponse(request) + message);
		else
			log.error(client.toString().toLowerCase() + ERROR_UNCONTROLLED + message);
	}
	 ...
	 * @param message
	 */
	public static void generatFinalLogSimple(ClientesServiciosEnum client, boolean isError, String message) {
		if(!isError)
			log.info(client.toString().toLowerCase().concat(". ") + message);
		else
			log.error(client.toString().toLowerCase() + ERROR_UNCONTROLLED + message);
	}
